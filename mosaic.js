...
function mosaic(imgIn, x, y) {
  ...
  for (let i = 0; i < imgIn.width; i += incr) {
    for (let j = 0; j < imgIn.height; j += incr) {
      ...
      let r = map(b, 0, 100, 0, incr); // radius dependsd on brightness
...
function postalImg(imgIn, x, y) {
  push();
  translate(x, y); // starting at top left of the output
  scale(2); // image output scaled by 2
  imgIn.loadPixels();
  noStroke();
  var incr = 4;

  /** loop through width and height of image */
  for (let i = 0; i < imgIn.width; i += incr) {
    for (let j = 0; j < imgIn.height; j += incr) {
      push();
      ...
      let n = noise(i, j); // noise depends on (i,j)
      let w = lerp(1, incr, n); 
      let v = createVector(mouseX - i, mouseY - j); // get the vector between mouse and (i,j)
      rotate(v.heading()); // rotate the rectangle by the 2d angle 

     /** sepia reference: https://stackoverflow.com/questions/1061093/how-is-a-sepia-tone-created*/

     // apply sepia
      let r,g,b, altR, altG, altB // old and new altered rgb
      r = c[0]
      g = c[1]
      b = c[2]
      ...
      /** width and height depending on noise */
